...
describe("Table Routes", () => {
  ...
  const UpdatedTableIdForTest = 32;
  const tableIdForTest2 = 311;
  const PreexistingtableIdForTest = 5;
  const CapacityForTest = 5;
  const CapacityForTest2 = 50;
  const NewCapacityForUpdate = 6;
  ...
  it("table id already exists", async () => {
    const response = await chai
      ...
      .set("Authorization", `Bearer ${validAccessToken}`)
      .send({
        table_id: PreexistingtableIdForTest,
        capacity: 4,
  ...
  it("should update new tableId based on table ID", async () => {
    const response = await chai
      .request(app)
      .put(`/table/${tableIdForTest}`)
      .set("Authorization", `Bearer ${validAccessToken}`)
      .send({
        table_id: UpdatedTableIdForTest,
        capacity: 4,
        availability_status: "Available",
      });
  ...
  it("New table id already exists while editing table details", async () => {
    const response = await chai
      .request(app)
      .put(`/table/${UpdatedTableIdForTest}`)
      .set("Authorization", `Bearer ${validAccessToken}`)
      .send({
        table_id: PreexistingtableIdForTest,
        capacity: 6,
  ...
  it("should update only table capacity based on tableID", async () => {
    const response = await chai
      .request(app)
      .put(`/table/${PreexistingtableIdForTest}`)
      .set("Authorization", `Bearer ${validAccessToken}`)
      .send({
        table_id: PreexistingtableIdForTest,
        capacity: NewCapacityForUpdate,
        availability_status: "Available",
      });
  ...
  it("should update table availability based on tableID", async () => {
    const response = await chai
      .request(app)
      .put(`/table/${PreexistingtableIdForTest}`)
      .set("Authorization", `Bearer ${validAccessToken}`)
  ...
  it("should delete a table based on ID", async () => {
    const response = await chai
      .request(app)
      .delete(`/table/${PreexistingtableIdForTest}`)
      .set("Authorization", `Bearer ${validAccessToken}`);
    expect(response).to.have.status(200);
    expect(response.body)
      .to.have.property("message")
